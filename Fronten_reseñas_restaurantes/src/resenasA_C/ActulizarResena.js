...
const ActualizarResena = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      console.error( nombreRestaurante, calificacion, fechaVisita, observaciones);
      const response = await fetch(`http://localhost:8552/api/resena/actualizar/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        },
        body: JSON.stringify({ nombreRestaurante, calificacion, fechaVisita, observaciones }), 
      });
      ...
      if (response.ok) {
        ...
        console.error('Error al actulizar la reseña:', response.statusText);
        setError('La reseña no se actulizo correctamente. Por favor, inténtelo de nuevo.');
    ...
    <div className="wrapper bg-white">
      ...
      <form className="pt-3" onSubmit={handleSubmit}>
        ...
        <div className="d-flex justify-content-center align-items-center">
          <button type="submit" className="btn btn-block btn-primary">Actualizar reseña</button>
        </div>
        {showModal && (
...
export default ActualizarResena;